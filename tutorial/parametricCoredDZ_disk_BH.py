...
# Numerical version
def MrContraBH(r, rhoss, rss, rhoH, rH, Rvir, Mh, MBH):  # R200 is used in the place of Rvir
    Rd=(1+np.sqrt(2))/1.678*rH
    MsHern = (2 * np.pi * rhoH * pow(rH, 3))
    Ms = MsHern + MBH
    fb = Ms / Mh
    if Ms > Mh:
        print("fb>1, exit")
    ...
    Mi = MtotNFW(rbar,rhoss,rss) + MsHern*MrDiskOverMs(rbar, Rd) + MBH
    def eq_to_solve(lnrf):
        rfbar = 0.85 * pow(np.exp(lnrf) / Rvir, 0.8) * Rvir
        return np.log(r)-lnrf - np.log(1. - fb + (MsHern*MrDiskOverMs(rfbar, Rd)+MBH)/Mi) 
...
ac_cache = {}
def rhoDZcore(r,c200,rss,tr,rhoH,rH,MBH): 
   ...
   if cache_key in ac_cache:
      ...
      for j in range(len(rticks)): 
         x,y = MrContraBH(rticks[j],rhoss,rss,rhoH,rH,Rvir,Mh,MBH) # should really be Rvir here...
