...
def calc_values(Vmax, Rvmax, c200, cvir, Mvir, horder): 
   ...
   for i in range(len(ht)):
      ...
      dtr=dt/tc1
      #if(i==0): 
      #   vmax1=fvmaxb(trx,c200,rss,rhoHx,rHx)
      #   rmax1=frmaxb(trx,c200,rss,rhoHx,rHx)
      if(trx<1.1): 
         #vmax1+=(fvmaxb(trx+dtr*0.5,c200,rss,rhoHx,rHx)-fvmaxb(max(trx-dtr*0.5,0),c200,rss,rhoHx,rHx))
         #rmax1+=(frmaxb(trx+dtr*0.5,c200,rss,rhoHx,rHx)-frmaxb(max(trx-dtr*0.5,0),c200,rss,rhoHx,rHx))
         trx+=dtr
      else: break 
   if(trx>1.1):
      trx=1.1
      if(dtr>0.05): print('Gravothermal evolution too fast, dtr: ',dtr)
   vmax1,rmax1=fvrmaxb(trx,c200,rss,rhoH,rH)
...
with open('output_SIDM_sigma0_100_w_100_baryon_n1000_DMO.txt', 'w') as file:
